...
Dominios <- c(200,300) # c(50,75,100,150,200,300)Tamaños de los dominios D
...
for (g in 1:length(Dominios)) {
   ...
   #sydk<-pdk
   pdk<-ppdk<-array(rep(0,3*D*I),dim=c(3,D,I))
   ...
   # Empieza el bucle de las simulaciones:
    ...
    while(i<I){
      ...
      # Calculo de los beta´s y las u´s mediante la funcion BETA.U.compo definida en Estimacion BETA.R)    
       ...
       ppdk <- MMEcompo(D,X,y,i)
				 for(d in 1:D){
				 	#sydk[1,d,i]<-sY1[d]
				 	#sydk[2,d,i]<-sY2[d]
				 	#sydk[3,d,i]<-sY3[d]
				 	for(ii in 1:3){
				 	pdk[ii,d,i]<-ppdk[[1]][,d][[ii]]
				 	#pdk[2,d,i]<-p2[d]
				 	#pdk[3,d,i]<-p3[d]
				  }	}
   ...
   # Calculo de errores y sesgos absolutos y relativos
  	 ...
  	 for(d in 1:D){ 
  	 	for(k in 1:3){
  	 		...
  	 		#EMdk[d,k]<-(mean((sydk[k,d,]-mud[k,d,])^2,na.rm=TRUE))^0.5
  	 		#BMdk[d,k]<-mean(sydk[k,d,]-mud[k,d,],na.rm=TRUE)
        Epidk[d,k]<-(mean((pidk.gorro[k,d,]-pi[k,d,])^2,na.rm=TRUE))^0.5
  	 		Bpidk[d,k]<-mean(pidk.gorro[k,d,]-pi[k,d,],na.rm=TRUE)
  	 		Epdk[d,k]<-(mean((pdk[k,d,]-pi[k,d,])^2,na.rm=TRUE))^0.5
  	 		Bpdk[d,k]<-mean(pdk[k,d,]-pi[k,d,],na.rm=TRUE)
   ...
   for(k in 1:3){
   	...
   	Epk[k]<-mean(Epdk[,k],na.rm=TRUE)
   	Bpk[k]<-mean(Bpdk[,k],na.rm=TRUE)
    ...
    if(g==999) {
        write.table(Emudk, file="Emudk_D50.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Epidk, file="Epidk_D50.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Bmudk, file="Bmudk_D50.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Bpidk, file="Bpidk_D50.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Epdk, file="Epdk_D50.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Bpdk, file="Bpdk_D50.txt", row.names=FALSE, col.names=FALSE)
    	  ...
    	  resultados <- data.frame(Bmuk,Bpik,Bpk,Emuk,Epik,Epk)#,Epebk,Bpebk,)
        #resultados <- as.data.frame(t(resultados))
        write.table(resultados, file="Resultados_Simulacion2_Compo_D50.txt", sep="\t")

    }
    if(g==9999) {
        write.table(Emudk, file="Emudk_D75.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Epidk, file="Epidk_D75.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Bmudk, file="Bmudk_D75.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Bpidk, file="Bpidk_D75.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Epidk, file="Epidk_D75.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Epdk, file="Epdk_D75.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Bpdk, file="Bpdk_D75.txt", row.names=FALSE, col.names=FALSE)
    	 ...
    	 # write.table(Bpebdk, file="Bpebdk_D75.txt", row.names=FALSE, col.names=FALSE)
    	  resultados <- data.frame(Bmuk,Bpik,Bpk,Emuk,Epik,Epk)#,Epebk,Bpebk,)
        #resultados <- as.data.frame(t(resultados))
        write.table(resultados, file="Resultados_Simulacion2_Compo_D75.txt", sep="\t")
    }
    if(g==99) {
        write.table(Emudk, file="Emudk_D100.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Epidk, file="Epidk_D100.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Bmudk, file="Bmudk_D100.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Bpidk, file="Bpidk_D100.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Bpdk, file="Bpdk_D100.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Epdk, file="Epdk_D100.txt", row.names=FALSE, col.names=FALSE)
    	  ...
    	  resultados <- data.frame(Bmuk,Bpik,Bpk,Emuk,Epik,Epk)#,Epebk,Bpebk,)
        #resultados <- as.data.frame(t(resultados))
        write.table(resultados, file="Resultados_Simulacion2_Compo_D100.txt", sep="\t")
    }
    
     if(g==9999) {
        write.table(Emudk, file="Emudk_D150.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Epidk, file="Epidk_D150.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Bmudk, file="Bmudk_D150.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Bpidk, file="Bpidk_D150.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Bpdk, file="BMdk_D100.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Epdk, file="Epdk_D150.txt", row.names=FALSE, col.names=FALSE)
    	 ...
    	 # write.table(Bpebdk, file="Bpebdk_D150.txt", row.names=FALSE, col.names=FALSE)
    	  resultados <- data.frame(Bmuk,Bpik,Bpk,Emuk,Epik,Epk)#,Epebk,Bpebk,)
        #resultados <- as.data.frame(t(resultados))
        write.table(resultados, file="Resultados_Simulacion2_Compo_D150.txt", sep="\t")

    }
    if(g==1) {
        write.table(Emudk, file="Emudk_D200.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Epidk, file="Epidk_D200.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Bmudk, file="Bmudk_D200.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Bpidk, file="Bpidk_D200.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Epdk, file="Epdk_D200.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Bpdk, file="Bpdk_D200.txt", row.names=FALSE, col.names=FALSE)
    	 ...
    	 # write.table(Bpebdk, file="Bpebdk_D200.txt", row.names=FALSE, col.names=FALSE)
    	  resultados <- data.frame(Bmuk,Bpik,Bpk,Emuk,Epik,Epk)#,Epebk,Bpebk,)
        #resultados <- as.data.frame(t(resultados))
        write.table(resultados, file="Resultados_Simulacion2_Compo_D200.txt", sep="\t")
    }
    if(g==2) {
        write.table(Emudk, file="Emudk_D300.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Epidk, file="Epidk_D300.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Bmudk, file="Bmudk_D300.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Bpidk, file="Bpidk_D300.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Epdk, file="Epdk_D300.txt", row.names=FALSE, col.names=FALSE)
    	  write.table(Bpdk, file="Bpdk_D300.txt", row.names=FALSE, col.names=FALSE)
    	 #write.table(Epebdk, file="Epebdk_D300.txt", row.names=FALSE, col.names=FALSE)
    	  ...
    	  resultados <- data.frame(Bmuk,Bpik,Bpk,Emuk,Epik,Epk)#,Epebk,Bpebk,)
       # resultados <- as.data.frame(t(resultados))
        write.table(resultados, file="Resultados_Simulacion2_Compo_D300.txt", sep="\t")
    }
